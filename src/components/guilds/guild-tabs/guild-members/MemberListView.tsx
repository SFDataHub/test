// src/components/guilds/guild-tabs/guild-members/MemberListView.tsx
...
function Avatar({
  ...
  const iconUrl = iconUrlForClass(memberClass, 40);
...
const RolePill = ({ role }: { role: string }) => {
  ...
  return (
    <span
      className={"px-2 py-0.5 rounded-full text-xs font-semibold border " + (m[role] || "")}
    >
      {role}
    </span>
  );
...
export default function MemberListView({
  ...
  return (
    <div className="rounded-2xl border overflow-hidden" style={{ borderColor: theme.border }}>
      <style>
        {`.no-scrollbar::-webkit-scrollbar{width:0;height:0}.no-scrollbar{scrollbar-width:none;-ms-overflow-style:none}`}
      </style>
      <div
        className="max-h=[520px] max-h-[520px] overflow-auto no-scrollbar"
      ...
      >
        ...
        {rows.map((m, i) => (
          <button
            ...
            className={
              ("w-full text-left px-3 py-3 grid items-center gap-3 transition " +
                (i % 2 ? "bg-slate-800/10" : "")) +
          ...
          >
            <Avatar
              name={m.name}
              memberClass={(m as any).class ?? (m as any).values?.Class ?? null}
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs truncate" style={{ color: theme.sub }}>
                {(m as any).class}
                {(m as any).server ? ` • ${(m as any).server}` : ""}
              </div>
            </div>
            <div className="justify-self-end">
              <RolePill role={(m as any).role} />
            </div>
            {rightCols(m)}
          </button>
        ))}
